...
void main(void){
	...
	vec3 L = normalize(u_light-v_world_vertex_pos);
	...
	vec3 ambient_color = texture_color.xyz*u_ambient;
	vec3 diffuse_color = texture_color.xyz*NdotL;
	vec3 specular_color = texture_color.xyz * pow(RdotE, 20);

	vec3 final_color = ambient_color + diffuse_color + specular_color*texture_spec;
